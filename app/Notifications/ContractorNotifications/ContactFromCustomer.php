...
class ContactFromCustomer extends Notification
{
    use Queueable;
...
*Спешим Вам сообщить*! Вам открыт доступ к контактным
данным заказчика по тендеру *"ремонт двух спален"*.

С деталями можно ознакомиться перейдя по ссылке под
сообщением!

[Посмотреть детали](http://customer.zrobleno.com.ua/tender)')
            ->file(public_path('images/bot_statuses/Customer-data-available.png'), 'photo');
    }
