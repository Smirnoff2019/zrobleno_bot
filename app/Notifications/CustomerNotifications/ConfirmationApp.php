...
class ConfirmationApp extends Notification
{
    use Queueable;
...
Мы *поздравляем Вас*! Ваша заявка на тендер *"Ремонт
двух комнат в новострое"* подтверждена и запущена.

С деталями можно ознакомиться перейдя по ссылке под
сообщением!

[Посмотреть детали](http://customer.zrobleno.com.ua/tender)')
            ->file(public_path('images/bot_statuses/Success.png'), 'photo');
    }
