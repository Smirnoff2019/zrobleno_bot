...
class FailedRestartTender extends Notification
{
    use Queueable;
...
*Спешим Вам сообщить*! Ваша попытка перезапуска тендера
*"Ремонт двух комнат в новострое"* потерпела *неудачу*.

С деталями можно ознакомиться перейдя по ссылке под
сообщением!

[Посмотреть детали](http://customer.zrobleno.com.ua/tender)')
            ->file(public_path('images/bot_statuses/Something-went-wrong.png'), 'photo');
    }
