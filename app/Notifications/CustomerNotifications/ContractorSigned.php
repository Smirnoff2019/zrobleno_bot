...
class ContractorSigned extends Notification
{
    use Queueable;
...
*Мы поздравляем Вас*! На Ваш тендер *"Ремонт двух
комнат в новострое"* подписался подрядчик.

С деталями можно ознакомиться перейдя по ссылке под
сообщением!

[Посмотреть детали](http://customer.zrobleno.com.ua/tender)')
            ->file(public_path('images/bot_statuses/New-Bidder.png'), 'photo');
    }
